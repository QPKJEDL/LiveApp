...
public class MessageBaseActivity extends BaseActivity {
    ...
    protected void loadUserName(IMessage msg) {
        ...
        TextBean text = new Gson().fromJson(msg.content.getRaw(), TextBean.class);
        final UserRoomBean bean = new Gson().fromJson(text.getText(), UserRoomBean.class);
        msg.setSenderAvatar(bean.getUser().getIcon());
        if (TextUtils.isEmpty(bean.getUser().getUsername())) {
            ...
            asyncGetUser(msg.sender, new MessageActivity.GetUserCallback() {
                ...
                public void onUser(MessageActivity.User u) {
                    fmsg.setSenderName(bean.getUser().getUsername());
                    fmsg.setSenderAvatar(bean.getUser().getIcon());
                }
            });
        } else {
            msg.setSenderName(bean.getUser().getUsername());
        }
    }
