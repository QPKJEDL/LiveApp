...
package com.qingpeng.mz.video.im;
...
public class MessageBaseActivity extends BaseActivity {
    ...
    protected static boolean hasLateMore;
...
//        resetMessageTimeBase();
    ...
    protected void loadUserName(IMessage msg) {
        ...
        UserRoomBean text = new Gson().fromJson(msg.content.getRaw(), UserRoomBean.class);
        msg.setSenderAvatar(text.getUser().getIcon());
        if (TextUtils.isEmpty(text.getUser().getUsername())) {
            msg.setSenderName(text.getText());
            ...
            asyncGetUser(msg.sender, new MessageActivity.GetUserCallback() {
                ...
                public void onUser(MessageActivity.User u) {
                    fmsg.setSenderName(text.getUser().getUsername() + " :" + text.getText());
                    fmsg.setSenderAvatar(text.getUser().getIcon());
                }
            });
        } else {
            msg.setSenderName(text.getUser().getUsername() + " :" + text.getText());
        }
    }
...
//        }
    ...
    public void sendTextMessage(String text, List<Long> at, List<String> atNames) {
        if (text.length() == 0) {
            return;
        }
